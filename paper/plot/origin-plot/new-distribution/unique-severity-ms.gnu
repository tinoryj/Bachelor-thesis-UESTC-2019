...
set xtics nomirror
set ytics nomirror
set key  maxrows 2 at screen 0.74, screen 0.55#for example
set ytics 5
set xrange [0:6]
set yrange [0:25]
...
plot 'unique-severity-ms' \
		 every ::0::0 using 1:($2*100) with boxes ls 1 title "WIN7", \
''		 every ::1::1 using 1:($2*100) with boxes ls 2 title "SERV-03", \
''		 every ::2::2 using 1:($2*100) with boxes ls 3 title "SERV-08", \
''		 every ::3::3 using 1:($2*100) with boxes ls 4 title "VISTA-B", \
''		 every ::4::4 using 1:($2*100) with boxes ls 5 title "VISTA-U"
...
unset key
plot 'unique-severity-ms' \
		 every ::0::0 using 1:($3*100) with boxes ls 1 notitle, \
''		 every ::1::1 using 1:($3*100) with boxes ls 2 notitle, \
''		 every ::2::2 using 1:($3*100) with boxes ls 3 notitle, \
''		 every ::3::3 using 1:($3*100) with boxes ls 4 notitle, \
''		 every ::4::4 using 1:($3*100) with boxes ls 5 notitle 
unset multiplot
